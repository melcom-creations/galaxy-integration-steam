...
from steam_network.protocol.steam_types import ProtoUserInfo  # Imported for user presence updates.
...
class SteamNetworkBackend(BackendInterface):
    ...
    async def pass_login_credentials(self, step, credentials: Dict[str, str], cookies):
        ...
        if (DisplayUriHelper.LOGIN.EndUri() in end_uri):
            ...
            return await self._handle_steam_guard_check(DisplayUriHelper.TWO_FACTOR_CONFIRM, True, **fallback_data)  # Return to the confirmation step.
        else:
    ...
    async def _handle_login_finished(self, credentials: Dict[str, str]) -> Union[NextStep, Authentication]:
        ...
        if (result == UserActionRequired.NoActionConfirmLogin):
            # This response still requires confirmation, so continue with the confirmation flow.
    ...
    # Confirmation flows call this directly after the user approves the login in Steam.
    # Edge cases still need to reopen the step if confirmation expires or fails.
    async def _handle_2FA_PollOnce(self, is_confirm : bool = False) -> UserActionRequired:
        ...
        # The poll can return either a completed login or a confirmation step that is still pending.
    ...
    async def _authenticate_with_stored_credentials(self, stored_credentials) -> Union[NextStep, Authentication]:
        ...
        # Save normalized credentials back to Galaxy when the stored format changes.
        if self._user_info_cache.is_initialized():
            migrated_creds = self._user_info_cache.to_dict()
            if migrated_creds != stored_credentials:  # The stored credential format changed.
